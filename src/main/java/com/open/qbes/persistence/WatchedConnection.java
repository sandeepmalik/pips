...
class WatchedConnection implements Connection {

    private static final Log log = Log.getLogger(WatchedConnection.class);
    ...
    WatchedConnection(Connection innerConnection) {
    ...
    public String toString() {
        return String.format("Connection[%d, ownerThread = %s]", serial, currentThread().getName());
    }
    ...
    public void close() throws SQLException {
        if (DB.TRANSACTION_FLAG.get() == null) {
            innerConnection.close();
            log.debug("Released connection %s", this);
        } else log.debug("Ignoring the close() call for connection %s as transaction is still running.", this);
    }
